...
Returns the profile status and next steps.`,
	{
		linux_username: z
			.string()
			.describe('The Linux username on the VPS (e.g., "alice")'),
		discord_user_id: z
			.string()
			.describe("The Discord user ID of the person requesting setup"),
	},
	async (args) => {
		const data = {
			type: "healer_setup",
...
server.tool(
	...
	"Add a repository for the code healer to monitor. The user must have a healer profile set up first.",
	{
		...
		local_path: z
			.string()
			.describe('Absolute path on the VPS (e.g., "/home/alice/projects/api")'),
		inspection_types: z
			.array(z.enum(["log-analysis", "code-review", "security-review"]))
			.describe("What types of inspection to run"),
		schedule: z
			.string()
			.optional()
			.describe('Cron expression (default: "0 23 * * *" = 11pm daily)'),
	},
	async (args) => {
		const data = {
			type: "healer_setup",
...
server.tool(
	...
	"Add a remote log source for the code healer. Logs are fetched via Tailscale SSH before each heal run.",
	{
		...
		host: z
			.string()
			.describe('Tailscale hostname or IP (e.g., "100.84.112.81")'),
		log_path: z
			.string()
			.describe('Absolute path on remote machine (e.g., "/var/log/api/")'),
		log_pattern: z
			.string()
			.optional()
			.describe('Glob pattern (default: "*.log")'),
